...
public class AddEdgeDialog extends JDialog {
    ...
    public AddEdgeDialog() {
        ...
        for (CellView cv : mg.getGraphLayoutCache().getAllViews()) {
            if (cv.getAllAttributes().get("name") != null) {
        ...
        ok.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (fromcell.getSelectedItem() == null || tocell.getSelectedItem() == null) {
                    ...
                    for (CellView cv : mg.getGraphLayoutCache().getAllViews()) {
                        if (cv.getAllAttributes().get("name") != null
                                && (cv.getAllAttributes().get("name").equals(fromcell.getSelectedItem().toString()))) {
                            edge.setSource(((DefaultMutableTreeNode) cv.getCell()).getChildAt(0));
                            MyGraphConstants.setPartent(edge.getAttributes(),
                                    (String) cv.getAllAttributes().get("name"));
                        }
                        if (cv.getAllAttributes().get("name") != null
                                && (cv.getAllAttributes().get("name").equals(tocell.getSelectedItem().toString()))) {
